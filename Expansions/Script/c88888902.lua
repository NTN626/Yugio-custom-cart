...
function s.cfilter(c)
	return not c:IsCode(id) and (c:IsSetCard(0x8b4) or c:IsType(TYPE_NORMAL)) and not c:IsPublic()
end
function s.confilter(c)
	return c:IsFaceup() and not c:IsType(TYPE_TOKEN)
		and (c:IsType(TYPE_NORMAL) or c:IsSetCard(0x8b4) and c:IsLevelAbove(5))
end
function s.cost(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if b1 then
		if b2 and not Duel.SelectYesNo(tp,aux.Stringid(29095457,0)) then return end
...
function s.setfilter(c)
	return c:IsFaceup() and c:IsSetCard(0x8b4)
end
