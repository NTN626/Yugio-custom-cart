...
function s.initial_effect(c)
	...
	-- Effect 1: Show up to 3 “NTN, The Captain of Hyperion” in your hand, face‑up Monster Zone, and/or GY;
	...
	-- target 1 banished Spell/Trap that mentions “NTN, The Captain of Hyperion”;
...
function s.bewdfilter(c)
	return c:IsCode(88888800) and (c:IsLocation(LOCATION_HAND) 
...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	local dg=Duel.SelectMatchingCard(tp,nil,tp,0,LOCATION_ONFIELD,ct,ct,nil)
	if #dg>0 then
		Duel.Destroy(dg,REASON_EFFECT)
	end
end

-- Effect 2: Graveyard Quick
...
function s.gravefilter(c)
	return c:IsFaceup() and c:IsType(TYPE_SPELL+TYPE_TRAP) 
		and aux.IsCodeListed(c,88888800) and c:IsAbleToDeck()
end
function s.gravetg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(s.gravefilter,tp,LOCATION_REMOVED,0,1,nil) end
