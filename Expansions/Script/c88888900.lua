...
function s.thfilter(c)
	return c:IsSetCard(0x8b4) and c:IsType(TYPE_MONSTER) and c:IsAbleToHand()
end
...
function s.sendfilter1(c)
	return (c:IsType(TYPE_NORMAL) or (c:IsSetCard(0x8b4) and c:IsType(TYPE_MONSTER)))
	   and c:IsAbleToGrave()
end
function s.sendfilter2(c)
	return c:IsSetCard(0x8b4) and c:IsType(TYPE_SPELL+TYPE_TRAP)
