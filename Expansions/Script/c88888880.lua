...
-- Search for a LIGHT Level 1 Tuner (except this card)
function s.thfilter(c)
	return c:IsType(TYPE_TUNER) and c:IsAttribute(ATTRIBUTE_LIGHT)
	   and c:IsLevel(1) and not c:IsCode(88888880)
	   and c:IsAbleToHand()
end
...
-- Discard self; send 1 Effect Monster you control to GY, then Special Summon 1 "SetCard 0x929" monster
...
function s.spfilter(c,e,tp)
	return c:IsSetCard(0x929) and c:IsLevel(8) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
function s.gvtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	local ft=Duel.GetLocationCount(tp,LOCATION_MZONE)
	if chkc then return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp)
		and s.gvfilter(chkc,ft) end
	if chk==0 then
		return ft>-1 and Duel.IsExistingTarget(s.gvfilter,tp,LOCATION_MZONE,0,1,nil,ft)
