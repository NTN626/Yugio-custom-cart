...
function s.initial_effect(c)
	-- Fusion Materials: "NTN, The Captain of Hyperion" + 1 Effect Monster on the field (either player)
	c:EnableReviveLimit()
	aux.AddFusionProcCodeFun(c,88888800,aux.FilterBoolFunction(Card.IsType,TYPE_EFFECT),1,true)
...
-- Contact Fusion material filter: NTN, The Captain of Hyperion or any Effect Monster
function s.cfilter(c)
	return (c:IsCode(88888800) or c:IsType(TYPE_EFFECT)) and c:IsAbleToGraveAsCost()
end
...
function s.resop(e,tp,eg,ep,ev,re,r,rp)
	...
	ex:SetTarget(function(e,c,sump,sumtype,sumpos,targetp,se)
		return c:IsLocation(LOCATION_EXTRA)
		   and (not c:IsRace(RACE_WARRIOR) or not c:IsAttribute(ATTRIBUTE_LIGHT))
	end)
